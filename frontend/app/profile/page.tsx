            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight">Profilim</h1>
                    </div>
                ...
                {!isEditing ? (
                    <button
                        ...
                        className="flex items-center justify-center gap-2 px-8 py-3.5 bg-slate-900 text-white font-bold rounded-2xl hover:bg-slate-800 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
                    >
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            disabled={isSaving}
                            className="flex items-center justify-center gap-2 px-6 py-3.5 bg-white border-2 border-slate-200 text-slate-700 font-bold rounded-2xl hover:bg-slate-50 transition-all"
                        >
                            <X size={18} /> İptal
                        </button>
                        <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-2">
                        ...
                        <div className={`p-6 rounded-3xl transition-colors ${isEditing ? 'bg-blue-50/50 border-2 border-blue-100' : 'bg-slate-50 border border-slate-100'}`}>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Ad Soyad</p>
                            </div>
                            {isEditing ? (
                                <input
                                    className="w-full px-4 py-3 bg-white border-2 border-blue-200 rounded-xl outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 font-bold text-slate-800 transition-all text-lg"
                                    value={formData.full_name}
                                    placeholder="Adınızı girin"
                                    onChange={(e) => setFormData({ ...formData, full_name: e.target.value })}
                                />
                            ) : (
                                <p className="text-slate-800 font-black text-xl px-2">
                                    {formData.full_name || "İsim Belirtilmemiş"}
                                </p>
                            )}
                        </div>
                        ...
                        <div className={`p-6 rounded-3xl transition-colors ${isEditing ? 'bg-purple-50/50 border-2 border-purple-100' : 'bg-slate-50 border border-slate-100'}`}>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">E-posta Adresi</p>
                            </div>
                            {isEditing ? (
                                <input
                                    type="email"
                                    className="w-full px-4 py-3 bg-white border-2 border-purple-200 rounded-xl outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 font-bold text-slate-800 transition-all text-lg"
                                    value={formData.email}
                                    placeholder="ornek@email.com"
                                    onChange={(e) => setFormData({ ...formData, email: e.target.value })}
                                />
                            ) : (
                                <p className="text-slate-800 font-black text-xl px-2 truncate">
                                    {formData.email || "E-posta Belirtilmemiş"}
                                </p>
                            )}
                        </div>
                    </div>
